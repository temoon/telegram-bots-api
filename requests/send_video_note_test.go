...
func TestSendVideoNote_GetValues(t *testing.T) {
	tests := []struct {
		name     string
	...
	}{
		{
			...
			request: &SendVideoNote{
				...
				VideoNote: telegram.NewInputFile("file_id_123", nil, ""),
			},
			expected: map[string]string{
				"chat_id":    "123456",
				"video_note": "file_id_123",
			},
			wantErr: false,
		},
		{
...
func TestSendVideoNote_GetFiles(t *testing.T) {
	t.Run("with file reader", func(t *testing.T) {
		reader := bytes.NewReader([]byte("fake file data"))
		request := &SendVideoNote{
			ChatId:    telegram.NewChatId(123456, ""),
			VideoNote: telegram.NewInputFile("", reader, "test.jpg"),
