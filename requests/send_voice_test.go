...
func TestSendVoice_GetValues(t *testing.T) {
	tests := []struct {
		name     string
		request  *SendVoice
	...
	}{
		{
			...
			request: &SendVoice{
				...
				Voice:  telegram.NewInputFile("file_id_123", nil, ""),
			},
			expected: map[string]string{
				"chat_id": "123456",
				"voice":   "file_id_123",
...
func TestSendVoice_GetFiles(t *testing.T) {
	t.Run("with file reader", func(t *testing.T) {
		reader := bytes.NewReader([]byte("fake file data"))
		request := &SendVoice{
			ChatId: telegram.NewChatId(123456, ""),
			Voice:  telegram.NewInputFile("", reader, "test.jpg"),
		}
